...
public class GameState
{
	...
	/// <param name="opponentName">Names of the computer players</param>
	...
	/// <returns>The stock to draw cards from</returns>
	public string PlayRound(Player player, Player playerToAsk, Values valueToAskFor, Deck stock)
	{
		string valuePlural = (valueToAskFor == Values.Six) ? "Sixes" : $"{valueToAskFor}s";
		string message = $"{player.Name} asked {playerToAsk.Name} for {valuePlural}{Environment.NewLine}";
		IEnumerable<Card> cards = playerToAsk.DoYouHaveAny(valueToAskFor, stock);
	...
	{
		int playerCards = Players.Select(player => player.Hand.Count()).Sum();
		if (playerCards > 0)
		{
			return "";
		}

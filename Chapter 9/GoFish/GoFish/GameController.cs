...
public class GameController
{
	...
	private void ComputerPlayersPlayNextRound()
	{
		...
		do
		{
			computerPlayersWithCards = gameState
			.Opponents
			.Where(player => player.Hand.Count() > 0);
			foreach (Player player in computerPlayersWithCards)
			{
				Player randomPlayer = gameState.RandomPlayer(player);
				Values randomValue = player.RandomValueFromHand();
				Status += gameState
				.PlayRound(player, randomPlayer, randomValue, gameState.Stock) + Environment.NewLine;
			}
